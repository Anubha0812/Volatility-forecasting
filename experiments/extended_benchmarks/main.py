...
output_folder="incremental_finetuning"
...
symbols=['.AEX', '.AORD', '.BFX', '.BVSP', '.DJI','.FCHI','.FTSE', '.GDAXI', '.HSI', '.IBEX','.IXIC', '.KS11','.KSE', '.MXX', '.N225','.RUT','.SPX', '.SSEC','.SSMI','.STI','.STOXX50E']
# #################################################################################################################################################
...
def process_symbol(symbol):
    ...
    csv_file_path = f'{symbol_cleaned}_vol_data.csv'
    ...
    while test_end < total_data_points:  # defin a loop that will extract the data and do the fine-turning
        ...
        CHECKPOINT_DIR = os.path.join('vol_incremental', symbol, f'iteration_{i}')
        i=i+1
